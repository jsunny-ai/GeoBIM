...
function buildBoundaryTargets(
  ...
  for (const p of profiles) {
    ...
    for (const seg of p.segs) {
      const r = STRATA_KEYS.indexOf(seg.type as StrataKey)
      if (r < 0) continue
      if (r > 0) {
        targets[STRATA_KEYS[r - 1]].push({ x: p.x, y: p.y, z: surfElev - seg.from })
      }
      targets[STRATA_KEYS[r]].push({ x: p.x, y: p.y, z: surfElev - seg.to })
    }
  }
  return targets
}
  ...
  try {
    ...
    const profiles = boreholes
      ...
      .map((b) => {
        ...
        return {
          x: b.longitude, y: b.latitude, elev: b.elevation, thick, deepestRank, segs, drilledDepth,
    ...
    if (boundarySnapDiag.maxAbsErrorRaw > CONTACT_TOLERANCE_M) {
      throw new Error(
        `시추공 지층 접촉 hard constraint 위반: 최대 오차 ${boundarySnapDiag.maxAbsErrorRaw.toFixed(6)}m`,
      )
    }
