...
def test_wide_format_detected_and_converted():
    ...
    # 매립층+풍화토 인접 토사층 병합 → 0~22.0
    assert bh1["strata"][0]["지층명"] == "토사"
    assert bh1["strata"][0]["상심도"] == 0.0
    assert bh1["strata"][0]["하심도"] == 22.0
...
def test_wide_missing_layer_excluded():
    ...
    bh1 = next(b for b in bhs if b["name"] == "BH-1")
    # 붕적/보통암/경암은 '-' → 적재 제외. 병합 후 토사/풍화암/연암 3개.
    assert [s["지층명"] for s in bh1["strata"]] == ["토사", "풍화암", "연암"]
...
def test_long_format_cp949_and_merge():
    ...
    assert len(bhs) == 1
    dh1 = bhs[0]
    # 매립토+퇴적층 → 토사 병합(0~8), 풍화암(8~15)
    assert [s["지층명"] for s in dh1["strata"]] == ["토사", "풍화암"]
    assert dh1["strata"][0]["하심도"] == 8.0
