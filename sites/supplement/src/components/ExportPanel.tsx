...
const LAYER_CONFIG = [
  { key: "soil",           label: "토사",   color: "#8B7355" },
  { key: "weathered_rock", label: "풍화암", color: "#C4A57B" },
  { key: "soft_rock",      label: "연암",   color: "#6B8E5A" },
  { key: "normal_rock",    label: "보통암", color: "#5F6552" },
  { key: "hard_rock",      label: "경암",   color: "#3D3D3D" },
]
    ...
    <div style={{ padding: "14px 16px" }}>
          ...
          return (
            ...
            >
              <span style={{
                ...
                background: checked ? color : C.btnIdle,
                border: `1px solid ${checked ? color : C.btnIdleBd}`,
              }} />
              <span style={{ fontSize: 12, color: C.secondary }}>{label} 하한면</span>
      ...
      <section style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 4 }}>
          {RES_OPTIONS.map((r) => (
            <button
              key={r}
              onClick={() => setOpts(prev => ({ ...prev, gridRes: r }))}
              style={{
      ...
      <button
        ...
        style={{
          ...
          fontSize: 14, fontWeight: 700, cursor: canExport && !newOnlyWarn && opts.layers.length > 0 ? "pointer" : "not-allowed",
          background: exportState === "done"
            ? C.green
            : exportState === "error"
            ? "rgba(232,83,58,.2)"
            : canExport && !newOnlyWarn && opts.layers.length > 0
            ? C.btnActive
            : C.btnIdle,
          border: `1px solid ${
          ...
          color: canExport && !newOnlyWarn && opts.layers.length > 0 ? "#fff" : C.tertiary,
