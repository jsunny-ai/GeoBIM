        ...
        <main style={{ padding: 18, overflowY: "auto", maxHeight: "90vh" }}>
          ...
          {rows.map((row, index) => (
            <div key={index} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1.3fr auto", gap: 6, marginBottom: 6 }}>
              ...
              <select style={input} value={row.soil_type} onChange={(e) => setRows((prev) => prev.map((r, i) => i === index ? { ...r, soil_type: e.target.value, strata_group: GROUPS[e.target.value] } : r))}>
                {Object.keys(GROUPS).map((name) => <option key={name}>{name}</option>)}
              </select>
