...
## 데이터 정합 규칙 (기존 PDF 데이터와 동일)

- 지층명은 `normalize_strata_group` 으로 한글 5대 분류(토사/풍화암/연암/보통암/경암)로 저장.
- 매립층·퇴적층·풍화토 등 세분류 → "토사" 로 통합하고, 인접 동일분류 층을 병합
  (경계 tol 0.05m, `pdf_service._merge_adjacent_strata_rows` 와 동일 규칙).
- 좌표는 WGS84로 변환해 저장, 원본 좌표계는 `meta_crs` 로 기록.
