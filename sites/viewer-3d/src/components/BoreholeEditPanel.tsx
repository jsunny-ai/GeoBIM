...
const soilTypes = ["토사", "풍화암", "연암", "보통암", "경암", "미분류"]
...
export const BoreholeEditPanel: React.FC<Props> = ({ borehole, onClose, onPreviewChange, onSaved }) => {
  ...
  }), [borehole, elevation, latitude, longitude, strata])
  ...
  const addRow = () => {
    ...
    setStrata((prev) => [...prev, { depth_top: lastBottom, depth_bottom: lastBottom + 1, soil_type: "토사" }])
  }
    ...
    <aside style={{
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 14 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
          ...
          <button onClick={addRow} style={smallButtonStyle}>+ 추가</button>
        </div>

        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {strata.map((row, index) => (
            <div key={index} style={{ display: "grid", gridTemplateColumns: "62px 62px 1fr 26px", gap: 6, alignItems: "center" }}>
              <input
                type="number"
                step="0.1"
                value={row.depth_top}
